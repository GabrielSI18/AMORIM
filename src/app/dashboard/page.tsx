...
async function DashboardPageContent() {
  ...
  if (!isAdmin) {
    ...
    return (
      ...
      >
        <div className="pt-4">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-[#E0E0E0] mb-2">
            Olá, {user.firstName || 'Usuário'}!
          </h2>
          <p className="text-gray-500 dark:text-[#A0A0A0] mb-8">Bem-vindo ao seu painel</p>

          <div className="bg-white dark:bg-[#1E1E1E] rounded-xl p-6 shadow-sm">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-[#E0E0E0] mb-4">
              Suas Viagens
            </h3>
            {userBookings.length > 0 ? (
              <div className="space-y-3">
                {userBookings.map((booking) => (
                  <div key={booking.id} className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-[#2A2A2A] rounded-lg">
                    <div className="flex-1">
                      <p className="text-gray-900 dark:text-[#E0E0E0] font-medium">{booking.package.title}</p>
  ...
  return (
    ...
    >
      ...
      <section className="flex flex-col gap-4">
        <h2 className="text-xl font-bold text-gray-900 dark:text-[#E0E0E0] tracking-tight">
      ...
      <section className="grid grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <p className="text-base font-bold text-gray-900 dark:text-[#E0E0E0]">Clientes</p>
          </div>
          <p className="text-3xl font-bold text-gray-900 dark:text-[#E0E0E0]">
            {totalClients.toLocaleString('pt-BR')}
          </p>
          <p className="text-sm text-gray-500 dark:text-[#A0A0A0]">Visualizar Clientes</p>
        </Link>
        ...
        >
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center size-10 rounded-full bg-purple-500/10">
              <Handshake className="w-5 h-5 text-purple-500 dark:text-purple-400" />
            </div>
            <p className="text-base font-bold text-gray-900 dark:text-[#E0E0E0]">Afiliados</p>
          </div>
          <p className="text-3xl font-bold text-gray-900 dark:text-[#E0E0E0]">
            {totalAffiliates.toLocaleString('pt-BR')}
          </p>
          <p className="text-sm text-gray-500 dark:text-[#A0A0A0]">Visualizar Afiliados</p>
        </Link>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <p className="text-base font-bold text-gray-900 dark:text-[#E0E0E0]">Pacotes</p>
          </div>
          <p className="text-3xl font-bold text-gray-900 dark:text-[#E0E0E0]">
            {publishedPackages}/{totalPackages}
          </p>
          <p className="text-sm text-gray-500 dark:text-[#A0A0A0]">Publicados / Total</p>
        </Link>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <p className="text-base font-bold text-gray-900 dark:text-[#E0E0E0]">Reservas</p>
          </div>
          <p className="text-3xl font-bold text-gray-900 dark:text-[#E0E0E0]">
            {paidBookings}/{totalBookings}
          </p>
          <p className="text-sm text-gray-500 dark:text-[#A0A0A0]">Pagas / Total</p>
        </Link>
      </section>
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-4 rounded-xl bg-white dark:bg-[#1E1E1E] p-4 shadow-sm">
          <div className="flex justify-between items-center">
            <div>
              <p className="text-sm text-gray-500 dark:text-[#A0A0A0]">Últimos 30 dias</p>
            ...
            <div className="text-right">
              <p className="text-sm text-gray-500 dark:text-[#A0A0A0]">Ocupação</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-[#E0E0E0]">{occupancyRate}%</p>
            </div>
          </div>
          ...
          {revenueLastMonth === 0 && (
            <div className="text-center py-6 border-t border-gray-200 dark:border-gray-700">
              <DollarSign className="w-12 h-12 text-gray-300 dark:text-[#333] mx-auto mb-2" />
              <p className="text-gray-500 dark:text-[#A0A0A0]">Nenhuma receita registrada ainda</p>
              <p className="text-sm text-gray-400 dark:text-[#666]">As vendas aparecerão aqui</p>
            </div>
          )}

          {/* Gráfico simples quando há dados */}
          {revenueLastMonth > 0 && (
            <div className="flex items-center gap-2 py-4 border-t border-gray-200 dark:border-gray-700">
    ...
    return (
      <DashboardShell title="Erro no Dashboard">
        <div className="space-y-4">
          ...
          <pre className="text-xs whitespace-pre-wrap break-all bg-gray-100 dark:bg-[#1E1E1E] border border-gray-200 dark:border-[#333] rounded-lg p-3 text-red-600 dark:text-red-300">
